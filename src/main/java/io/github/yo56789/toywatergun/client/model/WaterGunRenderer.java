...
public class WaterGunRenderer extends GeoItemRenderer<WaterGunItem> {
    public WaterGunRenderer() {
        ...
        addRenderLayer(new TankRenderLayer<>(this, "FluidLevel", Identifier.ofVanilla("textures/misc/white.png")));
        addRenderLayer(new TranslucentRenderLayer<>(this, "Tank", Identifier.of(ToyWaterGun.MOD_ID, "textures/item/water_gun.png")));
    }
    ...
    public void addRenderData(WaterGunItem animatable, RenderData relatedObject, GeoRenderState renderState) {
        ...
        renderState.addGeckolibData(WaterGunItem.TANK_LEVEL, relatedObject.itemStack().getOrDefault(TWGItems.FLUID_COMPONENT, 0));
    }
    ...
    public void preApplyRenderLayers(GeoRenderState renderState, MatrixStack poseStack, BakedGeoModel model, @Nullable RenderLayer renderType, VertexConsumerProvider bufferSource, @Nullable VertexConsumer buffer, int packedLight, int packedOverlay, int renderColor) {
        ...
        model.getBone("FluidLevel").ifPresent(fluid -> fluid.setScaleY(calcHeight(renderState.getOrDefaultGeckolibData(WaterGunItem.TANK_LEVEL, 0))));
